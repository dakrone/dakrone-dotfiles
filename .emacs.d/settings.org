...
;; make sure to `mkdir ~/.emacs_backups'
...
(add-hook 'emacs-lisp-mode-hook 'my/setup-semantic-mode)
#+END_SRC
...
(use-package prodigy
  ...
  (progn
    (prodigy-define-service
      ...
      :cwd "~/ies/elasticsearch-1.3.4/"
      :command "~/ies/elasticsearch-1.3.4/bin/elasticsearch"
      :tags '(work test es)
      :port 9200)
...
** aria2 (downloader) mode
Doesn't quite work yet, but it's getting there...

#+BEGIN_SRC emacs-lisp
(use-package aria2
  :bind (("C-h C-a" . aria2-downloads-list)))
#+END_SRC

** volatile-highlights
Highlights things that have recently undergone a change, such as yanking text
it, undoing, etc.
