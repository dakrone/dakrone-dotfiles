...
(defvar my/install-packages
  '(
    ...
    moe-theme nyan-mode flatui-theme smyx-theme monokai-theme
    subatomic-theme subatomic256-theme color-theme-sanityinc-tomorrow
    powerline

    ;; misc
    diminish gist scpaste async sx
    ...
    fill-column-indicator golden-ratio wc-mode eyebrowse vlf
    ...
    ;; highlighting
    idle-highlight-mode highlight-symbol highlight-numbers
    ...
    helm-gtags helm-ls-git helm-flycheck helm-flyspell
...
(defun my/dired-extract ()
  "Use the `se' utility to extract a file"
  (interactive)
  (save-window-excursion
    (dired-do-async-shell-command
     "se" current-prefix-arg
     (dired-get-marked-files t current-prefix-arg))))
#+END_SRC
...
(use-package dired
  ...
  (progn
    ...
    (setq ls-lisp-dirs-first t
          dired-listing-switches "-alh" ;; default is "-al"
    ...
    (define-key dired-mode-map (kbd "M-E") #'my/dired-extract)
...
I set the width to 100, because although it should be 80, I only want
highlighting past 100 columns, otherwise Java is unreadable.
...
*** modeline
Ehh.. I go back and forth, right now I'm using [[https://github.com/milkypostman/powerline][Powerline]].
...
(use-package magit
  ...
  (progn
    (when (and (eq system-type 'darwin)
               (file-exists-p  "/usr/local/bin/emacsclient"))
...
(use-package prodigy
  ...
  (progn
    ...
    (prodigy-define-service
      ...
      :cwd "~/ies/elasticsearch-1.5.0"
      :command "~/ies/elasticsearch-1.5.0/bin/elasticsearch"
      :tags '(work test es)
      :port 9200)

    (prodigy-define-service
      :name "Elasticsearch 1.4.4"
      :cwd "~/ies/elasticsearch-1.4.4"
      :command "~/ies/elasticsearch-1.4.4/bin/elasticsearch"
      :tags '(work test es)
      :port 9200)

    (prodigy-define-service
      :name "Elasticsearch 1.3.8"
      :cwd "~/ies/elasticsearch-1.3.8/"
      :command "~/ies/elasticsearch-1.3.8/bin/elasticsearch"
      :tags '(work test es)
      :port 9200)
    ...
    (prodigy-define-service
      :name "Kibana 4.0.1"
      :cwd "~/ies/kibana-4.0.1-darwin-x64"
      :command "~/ies/kibana-4.0.1-darwin-x64/bin/kibana"
      :tags '(work test kibana)
      :port 9200)))
#+END_SRC
...
** highlight-symbol
:PROPERTIES:
:ID:       5709E13F-A5A7-45BF-BF53-E9B4CB6C26ED
:END:
...
** highlight-numbers
:PROPERTIES:
:ID:       5E251415-CA89-4E5A-904D-59A794764491
:END:
Highlights numbers in source code, pretty simple.
...
(use-package log4j-mode
  :init (add-hook 'log4j-mode-hook #'my/turn-on-hl-line-mode))
#+END_SRC
