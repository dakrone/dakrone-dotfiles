...
(defvar my/install-packages
  '(
    ...
    ;; misc
    diminish gist async sx exec-path-from-shell bbdb symon
    ...
    vlf hydra shrink-whitespace ix quick-preview pdf-tools smartscan
    ...
    ;; org-mode
    org htmlize gnuplot-mode gnuplot org-alert org-present
...
(use-package org
  ...
  (progn
    ...
    (custom-set-variables '(org-export-backends '(ascii html latex beamer md)))
...
(use-package helm-config
  ...
  (use-package helm-files
    :config (setq helm-ff-file-compressed-list '("gz" "bz2" "zip" "tgz")))
...
** auto completion (company and auto-complete)
:PROPERTIES:
...
And the alternative, the auto-complete config:
...
** ws-butler
Cleans up whitespace, but only on the lines that I actually touched.
...
(use-package popwin
  ...
  (progn
    ...
    (defun my/popup-downloads ()
      ...
      (interactive)
      (if (get-buffer "*eshell downloads*")
          (switch-to-buffer "*eshell downloads*")
