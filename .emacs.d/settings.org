...
;;; ---- path manipulation
    ...
    (defun curr-dir-git-repo-dirty (pwd)
      "Is the `pwd' dirty from Git's perspective?"
      (interactive)
      (when (and (eshell-search-path "git")
        ...
        (let ((dirty-output
               (shell-command-to-string
                (concat "cd "
                        pwd
                        " && git diff-index --quiet HEAD -- || echo -n 'Ϟ'"))))
          (when (> (length dirty-output) 0)
            (propertize
             (concat " " dirty-output " ")
             'face `(:foreground "red"))))))
      ...
      (when (and (eshell-search-path "git")
        ...
        (let ((git-output
               (shell-command-to-string
                (concat "cd "
                        pwd
                        " && git symbolic-ref HEAD | cut -d'/' -f3-10"))))
          (propertize
           (concat "["
    ...
    (setq eshell-prompt-function
          (lambda ()
            (concat
             ...
             (or (curr-dir-git-repo-dirty (eshell/pwd)))
...
** org-trello
Interfacing with my org-trello account
...
(use-package prodigy
  ...
  (progn
    (prodigy-define-service
      :name "Elasticsearch 1.3.1"
      :cwd "~/ies/elasticsearch-1.3.1/"
      :command "~/ies/elasticsearch-1.3.1/bin/elasticsearch"
      :tags '(work test es)
      :port 9200)
